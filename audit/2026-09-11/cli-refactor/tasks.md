...
## SXR-CLI-04 — errors: source identity and complete text by default  *(DELIVERED 2026-09-11, awaiting review)*
...
## SXR-CLI-05 — cmds: a filter no longer changes session scope
...
- **after:** `--budget` and `--line-limit` reject negatives (exit 2), 0 means no trimming, an invalid `SXR_BUDGET`/`SXR_LINE_LIMIT` prints one stderr notice instead of being silently ignored, and one compact-view cap applies to every trimmed row of a view.
...
- **accept:** `show --budget -1` and `show --line-limit -1` exit 2; `--budget 0` prints whole text; `SXR_BUDGET=abc` prints a notice and uses the default; grep rows and `-C` windows use the same cap.
- **review:** the flag/env/default precedence table; complete views (`--full`, zooms, `--json`) still bypass the budget.
